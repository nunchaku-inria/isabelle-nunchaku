...
Examples featuring Nunchaku's "specialize" optimization.
*)

section {* Examples Featuring Nunchaku's \textit{specialize} Optimization *}
...
nunchaku [dont_box, dont_specialize, expect = none]
sorry
...
nunchaku [card nat = 2, card 'a = 1, expect = none]
nunchaku [card nat = 2, card 'a = 1, dont_box, expect = none]
nunchaku [card nat = 2, card 'a = 1, dont_specialize, expect = none]
nunchaku [card nat = 2, card 'a = 1, dont_box, dont_specialize, expect = none]
sorry
...
lemma "\<forall>a. g a = a
       \<Longrightarrow> \<exists>b\<^sub>1 b\<^sub>2 b\<^sub>3 b\<^sub>4 b\<^sub>5 b\<^sub>6 b\<^sub>7 b\<^sub>8 b\<^sub>9 b\<^sub>10 (b\<^sub>11::nat).
           ...
           \<and> f5 g x = f5 (\<lambda>a. if b\<^sub>1 \<ge> b\<^sub>11 then
                                ...
                                h b\<^sub>2 + h b\<^sub>3 + h b\<^sub>4 + h b\<^sub>5 + h b\<^sub>6 + h b\<^sub>7 + h b\<^sub>8
                                + h b\<^sub>9 + h b\<^sub>10) x"
nunchaku [card nat = 2, card 'a = 1, expect = potential]
nunchaku [card nat = 2, card 'a = 1, dont_box, expect = potential]
nunchaku [card nat = 2, card 'a = 1, dont_specialize, expect = potential]
nunchaku [card nat = 2, card 'a = 1, dont_box, dont_specialize,
         expect = potential]
oops
